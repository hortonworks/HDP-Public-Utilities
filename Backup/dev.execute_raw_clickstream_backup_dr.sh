#!/bin/bash
# Utility to run a copy of the snapshots of the raw clickstream data from a source cluster and drop it to target cluster
...
hadoop distcp -update -prbugp -m 32 \
        hdfs://qa_namenode_host/datalake/raw/clickstream/.snapshot/$now \
